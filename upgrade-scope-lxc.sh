...
lxc init ubuntu:18.04 ${DU_LXC_IMG_UPGR}
...
lxc file push --recursive root ${DU_LXC_IMG_UPGR}/
cd .. && rm -Rf tmp
...
lxc exec ${DU_LXC_IMG_UPGR} -- bash -c "apt-get update && apt install -y \
  libboost-all-dev \
  libusb-1.0-0-dev \
  doxygen \
  python3-docutils \
...
lxc exec ${DU_LXC_IMG_UPGR} -- bash -c "cd /root \
...
echo "Cloning srsGUI"
lxc exec ${DU_LXC_IMG_UPGR} -- bash -c "cd /root/radio_code \
  && rm -Rf srsGUI \
...
echo "Building SCOPE"
lxc exec ${DU_LXC_IMG_UPGR} -- bash -c "cd /root/radio_code/srsLTE \
  && mkdir -p build \
  && cd build \
  && make clean \
  && cmake .. \
...
lxc exec ${DU_LXC_IMG_UPGR} -- bash -c "sed -i 's/^#*dl_freq\s*=\s*[[:alnum:]]*\s*$/dl_freq = 2435000000/g' /root/radio_code/srslte_config/enb.conf \
...
echo "Saving image"
lxc stop ${DU_LXC_IMG_UPGR}
lxc publish du-scope-1804 --alias ${DU_LXC_IMG_UPGR}
